    ...
    describe('>>> Box vs Box', () => {
        ...
        */
        it('Should not collide if they aren\'t touching', () => {
            const box1 = new BoxCollision({ x: 25, y: 25 }, { height: 25, width: 25 })
            const box2 = new BoxCollision({ x: 75, y: 75 }, { height: 25, width: 25 })
        ...
        */
        it('Should collide if a corner is located inside one of the boxes', () => {
            const box1 = new BoxCollision({ x: 20, y: 20 }, { height: 20, width: 20 })
            const box2 = new BoxCollision({ x: 5, y: 5 }, { height: 20, width: 20 })
            const box3 = new BoxCollision({ x: 35, y: 5 }, { height: 20, width: 20 })
            const box4 = new BoxCollision({ x: 5, y: 35 }, { height: 20, width: 20 })
            const box5 = new BoxCollision({ x: 35, y: 35 }, { height: 20, width: 20 })
        ...
        */
        it('Should collide if one completely goes through the other', () => {
            const box1 = new BoxCollision({ x: 20, y: 20 }, { height: 50, width: 20 })
            const box2 = new BoxCollision({ x: 10, y: 30 }, { height: 30, width: 100 })
            const result = box1.isColliding(box2)
        ...
        */
        it('Should not collide if they aren\'t touching but one is centered in the other but still not touching', () => {
            const box1 = new BoxCollision({ x: 484, y: 680 }, { height: 56, width: 56 })
            const box2 = new BoxCollision({ x: 17, y: 17 }, { height: 6, width: 990 })
            const result = box1.isColliding(box2)
            chai.expect(result).to.equal(false)
        })
    })
